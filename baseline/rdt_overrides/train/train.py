...
def train(args, logger):
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        pin_memory=True,
        # persistent_workers=False so 16 worker processes get torn down at every
        # epoch boundary (~924 steps at batch=16). Bounds per-worker memory growth
        # from imgaug==0.4.0's stochastic-kernel caches, which OOM-killed a prior
        # run at MaxRSS=1925G after ~14000 steps with persistent_workers=True.
