"""Interrupt variant for ``blocks_ranking_rgb``.

The robot sorts the RGB cubes, but a human avatar inspects one randomly chosen
block mid-task.  The robot first approaches that block to show intent, retreats
while the avatar picks it up with the Inspect motion, sorts the other two
blocks, then re-reads the inspected block's released pose and sorts it.
"""
...
class BlocksRankingRGBInterrupt(InspectMotionMixin, BlocksRankingRGB):
    """Human inspects one block while the robot is ordering RGB blocks."""

    INSTRUCTION = (
        "place the red, green, and blue blocks in left-to-right order; "
        "wait while the human inspects one block, then continue"
    )
    ...
    def load_actors(self):
        ...
        if target_rng is None:
            color_idx = int(np.random.randint(len(colors)))
        else:
            color_idx = int(target_rng.integers(0, len(colors)))
        self._interrupted_color = colors[color_idx]
        xys = self._sample_spawn_xys_for_order(order)
    ...
    def _sample_spawn_xys_for_order(self, order):
        ...
        for _ in range(self.SPAWN_SAMPLE_ATTEMPTS):
            ...
            for i, _color_name in enumerate(order):
        ...
        raise RuntimeError("[blocks_rgb_interrupt] could not sample non-overlapping cubes")
    ...
    def _calibrate_avatar_for_inspect(self):
        cube = self.blocks[self._interrupted_color]
    ...
    def play_once(self) -> bool:
        ...
        int_color = self._interrupted_color
        int_cube = self.blocks[int_color]
        non_interrupted = [c for c in ("red", "green", "blue") if c != int_color]
        ...
        for color_name in before_interrupt:
            ok_all = place_block(color_name) and ok_all
        ...
        for color_name in during_interrupt:
            ok_all = place_block(color_name) and ok_all
        ...
        try:
            ok = self._pick_and_place_block(int_color, arm_tag)
        except Exception:
            ok = False
        if not ok:
            ok_all = False
    ...
    def _eval_step_avatar(self):
        ...
        color_name = self._find_closest_table_block()
