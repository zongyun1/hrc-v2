"""Put-object-in-cabinet interrupt scaffold (single Franka + avatar).

RoboTwin's ``put_object_cabinet_interrupt`` is bimanual: one arm opens a drawer,
...
from ..avatar.inspect_motion_mixin import InspectMotionMixin, INSPECT_MOTION_SPECS
...
# referenced via the ``cabinet_drawer_items_interrupt`` category set on the
# class below.  (close_value for 071_can is applied inline at PickSpec.)
...
class PutObjectCabinetInterrupt(InspectMotionMixin, TopDownPickPlaceMixin, BaseTask):
    """Robot opens the top drawer of a cabinet, picks an object off the
    table after a human inspection, and drops it inside the open drawer."""

    INSTRUCTION = "place the object inside the cabinet drawer after the human inspects it"
    ...
    # Human inspection can leave tall objects tilted in the robot grasp.
    # Release with the payload already inside the opening but the gripper
    # above the rim, letting gravity finish the placement without a jaw/rim
    # collision during a deep descent.
    ...
    # Elevated front-right view — the old [0.50, 0.30, 1.30] camera was
    # blocked by this variant's avatar leaning over the front table edge
    # (keep in sync with task_bases/put_object_cabinet.py).
    ...
    # Avatar approaches from the +Y side of the table, facing -Y.  This
    # keeps the body out of the back-right cabinet/drop corridor while
    # still letting the right hand reach the front table object.
    ...
    # Use the shorter inspect clip by default so calibration/rollout stays
    # tractable; randomize_inspect can still sample the legacy long clip.
    INSPECT_POOL = [
        INSPECT_MOTION_SPECS["Inspect2"].as_pool_entry(),
        INSPECT_MOTION_SPECS["Inspect1"].as_pool_entry(),
        INSPECT_MOTION_SPECS["Inspect4"].as_pool_entry(),
    ...
    INSPECT_MOTION_NAME = "Inspect2"
    INSPECT_ATTACH_FRAME = 31
    INSPECT_DETACH_FRAME = 84
    AVATAR_MOTION_SLOW = 10.0
    AVATAR_HAND_ID = 1
    AVATAR_EXTRA_Z = 0.04
    # Clear the hand from the released object before the robot closes around
    # it.  The object detaches at its authored frame; only the avatar pose
    # transitions back to idle over these frames.
    INSPECT_RETURN_TO_IDLE_FRAMES = 30

    # Eval-mode interrupt schedule.  In eval mode the scripted
    # ``play_once`` flow is not used; a policy drives the robot through
    # repeated ``take_action`` calls, and this task fires the avatar
    # interrupt once at a random policy step.
    ...
    def __init__(self, config: dict = None):
        ...
        self.EVAL_TRIGGER_STEP_MIN = int(cfg.get(
            "eval_trigger_step_min", self.EVAL_TRIGGER_STEP_MIN))
        self.EVAL_TRIGGER_STEP_MAX = int(cfg.get(
            "eval_trigger_step_max", self.EVAL_TRIGGER_STEP_MAX))
        self.EVAL_GLIDE_POLICY_STEPS = int(cfg.get(
    ...
    def _sample_object_spec(self) -> dict:
        ...
        # ``cabinet_drawer_items_interrupt``.
    ...
    def load_actors(self):
        ...
        for sub in sorted(cabinet_dir.iterdir()):
            if sub.is_dir() and (sub / "mobility.urdf").exists():
                ...
                break
        if urdf is None:
            raise RuntimeError("[put_object_cabinet_interrupt] cabinet URDF not found")
        ...
        # The source drawer mesh has an unreliable/thin collision floor. Add
        # its hidden support after the payload so the established grasp body's
        # solver ordering is preserved, then park it until the drawer opens.
    ...
    def _get_object_world_center(self, actor, object_name, model_id=0):
        # Primitive origin == geometric centre (no mesh-origin model_data);
        # meshes fall through to the mixin's offset-corrected center.
        ...
        # Genesis-1.0.0 z-up GLB load: when the live AABB centre disagrees with
        # the model_data centre by >1.5 cm in z (e.g. 100_seal lies in a frame
        # model_data doesn't predict), trust the simulator AABB so the top-down
        # grasp targets the real object instead of air.  No-op for well-loaded
        # meshes — see base put_object_cabinet for the full rationale.
        try:
    ...
    def _pick_drawer(self):
        ...
        if not prismatic:
            raise RuntimeError("[put_object_cabinet_interrupt] no drawer links found")
        ...
        self._drawer_held_qpos = None
        self._object_grasp_reference_quat = np.asarray(
            to_numpy(self.object.entity.get_quat()), dtype=float,
        ).reshape(-1)[:4].copy()
        print(
    ...
    def _object_relative_grasp_yaw(self) -> float:
        reference = getattr(self, "_object_grasp_reference_quat", None)
        if reference is None:
            return 0.0
        current = np.asarray(
            to_numpy(self.object.entity.get_quat()), dtype=float,
        ).reshape(-1)[:4]
        return self.relative_parallel_jaw_yaw(current, reference)

    def _release_object_from_avatar(self) -> None:
        if self.avatar is not None:
            self.avatar.robot.detach_object(self.AVATAR_HAND_ID)
    ...
    def _hold_drawer_open(self) -> bool:
        ...
        if ok:
            self._sync_drawer_floor_proxy()
        return ok

    def _sync_drawer_floor_proxy(self) -> None:
        proxy = getattr(self, "_drawer_floor_proxy", None)
        if proxy is None or self._drawer_link is None or self._drawer_floor_z is None:
            return
        self.sync_support_proxy(proxy, self._drawer_link, self._drawer_floor_z)
    ...
    def _dryrun_capture_palm(self):
        ...
        # immediately after that frame instead of playing the full inspect
        # clip, then reset() clears the avatar action state.
    ...
    def _calibrate_avatar_for_inspect(self):
        if self.object is None:
            return
        target_palm = self._get_object_world_center(
        ...
        # Keep the shifted avatar on the +Y/front side of the table and
        # away from the cabinet body.  These are warnings, not hard stops,
        # so seeded layouts remain debuggable if a pose is marginal.
        if shifted_pos[1] < 0.18:
            pass
        if abs(shifted_pos[0] - self._cabinet_pos[0]) < 0.20:
            pass
    ...
    def _retreat_safe(self, arm_tag: str):
        ...
        if self._move_screw(retreat_pos, arm_tag) is None:
            link = tcp_to_link_pose(self._top_down_tcp(retreat_pos), arm.tcp_offset)
            self._move_seeded(link.to_pose7(), arm_tag)
    ...
    def _eval_step_avatar(self):
        ...
        if self._eval_glide_active:
            ...
            if self._eval_glide_remaining <= 0:
                self._eval_glide_active = False
                target = self._eval_target_object
                obj_pos = self._get_object_world_center(
                    target, self._object_id, self._object_model_id,
                )
                if not self._is_object_on_table(obj_pos):
                    return
                self._play_inspect_motion(
                    attach_obj=target.entity,
        ...
        shift = obj_pos - self._eval_palm_default
        shift[2] += self.AVATAR_EXTRA_Z
        target_avatar_pos = (
        ...
        self._eval_target_object = self.object

    def _is_object_on_table(self, obj_pos: np.ndarray) -> bool:
        if obj_pos[2] < self.TABLE_TOP_Z - 0.05:
            return False
        if obj_pos[2] > self.TABLE_TOP_Z + 0.35:
            return False
        if self._drawer_link is not None:
            metrics = self._object_in_drawer_metrics()
            if bool(metrics.get("object_in_drawer", False)):
                return False
        return True
    ...
    def play_once(self) -> bool:
        ...
        # Phase B — show intent on the table object, then let the avatar
        # inspect it before the robot commits to grasping.
        def _object_center():
            return self.live_entity_aabb_center(self.object.entity)
        ...
        if self.avatar is not None:
            obj_center = _object_center()
            intent_z = self.TABLE_TOP_Z + 0.20
            intent_pos = np.array([obj_center[0], obj_center[1], intent_z])
            intent_link = tcp_to_link_pose(
                self._top_down_tcp(intent_pos),
                self.robot.get_arm(arm_tag).tcp_offset,
            )
            self.open_gripper(arm_tag)
            if self._move_seeded(intent_link.to_pose7(), arm_tag) is None:
                pass

            self._play_inspect_motion(
                attach_obj=self.object.entity,
                hand_id=self.AVATAR_HAND_ID,
                attach_frame=self._attach_frame_scaled,
                detach_frame=self._detach_frame_scaled,
                return_to_idle_after=self.INSPECT_RETURN_TO_IDLE_FRAMES,
            ...
            # The authored inspect clip has already detached the object at its
            # release frame.  Leave it physical and settled here; retaining a
            # kinematic hand constraint during robot finger closure can inject
            # contact energy and expel the object when that constraint drops.
        else:
            pass

        # Phase C — pick the object from its latest pose and drop into the
        # open drawer via the mixin's ``pick_and_place`` helper.
        ...
        def _release_avatar_and_boost_grip():
            self._release_object_from_avatar()
            self._boost_finger_pd(arm_tag)
        ...
        pick = PickSpec(
            ...
            get_grasp_yaw=self._object_relative_grasp_yaw,
            after_close=_release_avatar_and_boost_grip,
            require_lift=True,
            max_grasp_offset=0.10,
    ...
    def play_blind_once(self) -> bool:
        ...
        def driven_step_sim():
            orig_step_sim()
            counters["sim"] += 1
            if counters["sim"] % tick_interval == 0 and bool(self.config.get("eval_mode", False)):
                self._eval_step_avatar()
