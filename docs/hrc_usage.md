...
完整模块职责见 [当前代码架构](architecture.md)。
...
Lightwheel 源码与 Lab 3 适配已经在仓库根目录，默认使用当前 checkout。
根目录 `install.sh` 安装当前 Lightwheel 和 vendored Arena 到指定 Lab 3 容器。
上游 LFS 资产准备见 [根 README](../README.md)。不再需要独立迁移目录或设置
`LIGHTWHEEL_ROOT`；该环境变量只用于显式覆盖源码路径。
...
- [人体接口 GPU 验证记录](lightwheel_interface_validation.md)
- [已有脚本避人导航与白色机械臂视频](../tools/isaaclab3/HUMAN_NAVIGATION.md)
