...
```bash
sbatch tools/isaaclab3/run_human.sbatch
```

输出 `outputs/lightwheel/human_JOB_ID/`：结果 JSON、逐控制步轨迹、视频、首尾帧、停让/恢复截图。
启动脚本显式检查结果状态，仿真 shutdown 即使吞掉 Python 异常也不会报成功。

```bash
python -m unittest tools.isaaclab3.test_navigation tools.isaaclab3.test_migrate isaac_human.test_motion
```
