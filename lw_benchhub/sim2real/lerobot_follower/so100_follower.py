...
class SO100Follower():
    """A SO100 Leader device for SE(3) control.
    """

    def __init__(self, port: str = '/dev/ttyACM0', recalibrate: bool = False, calibration_file_name: str = 'so100_follower.json',
    ...
    def __str__(self) -> str:
        ...
        msg = "SO100-Follower device for SE(3) control.\n"
        msg += "\t----------------------------------------------\n"
        msg += "\tMove SO100-Follower to control SO100-Follower\n"
        msg += "\tIf SO100-Follower can't synchronize with SO100-Follower, please add --recalibrate and rerun to recalibrate SO100-Follower.\n"
        return msg
    ...
    def disconnect(self):
        if not self.is_connected:
            raise DeviceNotConnectedError("SO100-Follower is not connected.")
        self._bus.disconnect()
        print("SO100-Follower disconnected.")

    def connect(self):
        if self.is_connected:
            raise DeviceAlreadyConnectedError("SO100-Follower is already connected.")
        self._bus.connect()
        self.configure()
    ...
    def calibrate(self, norm_mode_body):
        ...
        print("\n Running calibration of SO100-Follower")
        self._bus.disable_torque()
        for motor in self._bus.motors:
            self._bus.write("Operating_Mode", motor, OperatingMode.POSITION.value)

        input("Move SO100-Follower to the middle of its range of motion and press ENTER...")
