> Historical Genesis documentation. The active benchmark is described in [README.md](README.md).

# Genesis HR Bench

> Lightwheel 新版：[NavigateKitchen + TRUMANS 避人导航](lw_benchhub_lab3/HUMAN_NAVIGATION.md)。

> RoboCasa365 → Isaac Lab：[三任务迁移原型、实测结果与视频](robocasa_migration/README.md)。
> 新增：[coworker Phase 1 → Isaac Lab：厨房、PandaOmron 与人体共仿真](isaac_human/KITCHEN.md)。
> 新增：[ServeTea 场景、PandaOmron 与杯碟接触判定迁移](robocasa_migration/SERVE_TEA.md)。

> Human motion 升级方案（设计阶段）：[公开代码技术路线与实验安排](docs/human_motion_technical_routes.zh-CN.md)。

> 新增：[Ridgeback + Franka 仓储移动交接](isaac_kitchen/WAREHOUSE.md)。
> 当前 Isaac Lab 厨房、Franka 与 human avatar 的开发进度见
> [项目进度](isaac_kitchen/PROGRESS.md)，运行入口见
> [Isaac Lab README](isaac_kitchen/README.md)。以下为旧 Genesis 版本说明。
